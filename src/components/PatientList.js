...
const PatientList = ({
  ...
  const handleRemoveMedicine = (index) => {
    ...
    const handleSubmit = () => {
      ...
      Linking.openURL(`https://server.zyngchat.com/api/meetings/${meeting.id}/download-prescription`).catch(err => console.error("Couldn't load page", err));
          ...
          <View>
              ...
              <Modal isVisible={isModalVisible} style={styles.container}>
                <View style={{ flex: 1, backgroundColor: 'white', borderRadius: 10, padding: 20 }}>
                  ...
                  <TextInput style={customInputStyle.input} placeholder="Title" value={newMedicine.title} onChangeText={(text) => setNewMedicine({ ...newMedicine, title: text })} />
                  <TextInput style={customInputStyle.input} placeholder="Dose" value={newMedicine.dose} onChangeText={(text) => setNewMedicine({ ...newMedicine, dose: text })} />
                  <TextInput style={customInputStyle.input} placeholder="Instruction" value={newMedicine.dose_instruction} onChangeText={(text) => setNewMedicine({ ...newMedicine, dose_instruction: text })} />
                  <TextInput  style={customInputStyle.input} placeholder="Duration" keyboardType="numeric" value={newMedicine.duration.toString()} onChangeText={(text) => setNewMedicine({ ...newMedicine, duration: text })} />
